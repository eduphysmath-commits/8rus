...
# --- 1. НАСТРОЙКИ БАЗЫ (Оставляем те же, чтобы данные шли в одну таблицу) ---
...
if st.session_state.submitted:
    ...
    if name:
        # ANTI-CHEAT JS (Текст на русском)
        components.html(f"""
            <script>
                    ...
                    setTimeout(function() {{
                        if (document.hidden && !isSubmitting) {{
                            const payload = {{
                                ...
                                status: "cheated",
                                lang: "ru",
                                ai_feedback: "🚫 РАБОТА АННУЛИРОВАНА: Вы переключились на другое окно (Анти-чит)."
                            }};
        ...
        with st.form("exam_8_physics_ru"):
            ...
            q11 = st.text_area("11. Почему металлическая ложка кажется холоднее деревянной при одинаковой температуре?", height=70, placeholder="Напишите ваш ответ...")
            q12 = st.text_area("12. Как изменится сила Кулона, если расстояние между двумя зарядами увеличить в 3 раза?", height=70, placeholder="Напишите решение или ответ...")

            st.subheader("📍 РАЗДЕЛ С: Решение задачи (4 балла)")
            q13 = st.text_area("13. Задача: r = 10 см, q1 = 2*10^-7 Кл, q2 = 5*10^-7 Кл. Найдите силу взаимодействия (F):", height=100, placeholder="Покажите решение и ответ...")
...
search_query = st.text_input("Введите имя для поиска (например: Иван):", key="search_input")
            ...
            for data in results:
                with st.container():
                    ...
                    if data['status'] == 'cheated':
                        ...
                        st.warning("⏳ Работа еще проверяется... Попробуй позже.")
                    else:
                        col_score, col_fb = st.columns([1, 3])
                        with col_score:
                            st.metric("Итоговый балл", f"{data.get('score', 0)} / 20")
                        with col_fb:
                            with st.expander("📝 Обратная связь от учителя (AI)", expanded=True):
                                st.write(data.get('ai_feedback', 'Отзыв готовится...'))
                    st.markdown("<br>", unsafe_allow_html=True)
        else:
            st.info("ℹ️ Работа с таким именем не найдена.")